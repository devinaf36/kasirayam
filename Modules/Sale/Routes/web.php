...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::get('/sales/pdf/{id}', function ($id) {
        ...
        $dompdf->loadView('sale::print', [
            'sale' => $sale,
            'customer' => $customer,
        ]);
        $dompdf->setPaper('a4');

        return $dompdf->stream('sale-' . $sale->reference . '.pdf');
    })->name('sales.pdf');
    ...
    Route::get('/sales/pos/pdf/{id}', function ($id) {
        ...
        $dompdf = App::make(Dompdf::class);
        $dompdf->loadView('sale::print-pos', [
            'sale' => $sale,
        ]);
        $dompdf->setPaper('a7');
        $dompdf->setOption('margin-top', 8);
        $dompdf->setOption('margin-bottom', 8);
        $dompdf->setOption('margin-left', 5);
        $dompdf->setOption('margin-right', 5);

        return $dompdf->stream('sale-' . $sale->reference . '.pdf');
    })->name('sales.pos.pdf');


    //Generate Barrvdh Dompdf
    // Route::get('/sales/pdf/{id}', function ($id) {
    //     $sale = \Modules\Sale\Entities\Sale::findOrFail($id);
    //     $customer = \Modules\People\Entities\Customer::findOrFail($sale->customer_id);
