...
class HomeController extends Controller
{
    ...
    public function salesExpensesChart() {
        abort_if(!request()->ajax(), 404);

        $sales = $this->salesChartData();
        $expenses = $this->expensesChartData();

        return response()->json(['sales' => $sales, 'expenses' => $expenses]);
    ...
    public function expensesChartData() {
        $dates = collect();
        foreach (range(-6, 0) as $i) {
            $date = Carbon::now()->addDays($i)->format('d-m-y');
            $dates->put($date, 0);
        ...
        $days = [];
        foreach ($dates as $key => $value) {
            $data[] = $value / 100; // Adjust the division factor if needed
            $days[] = $key;
        }
